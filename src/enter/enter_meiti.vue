    ...
    <div class="qunmeiti">
      ...
      <ul>
        ...
        <li id="flag">
          ...
          <input readonly="readonly" onfocus='this.blur()' style="width: 26%;" ref="stylewidth" @click="addBtnFlag && diqu()" placeholder="请选择" v-model="addhanldiqu" />
        </li>
        <li id="addbtn">
          <span></span>
        ...
        <li>
          <i class="diandiant"></i>
          <span>组织机构代码</span>
          <input id="zuzhi" type="text" v-model="organizational_code" />
      ...
      <ol>
        <li>
          ...
          <div class="bgeba">2～10个字，请勿使用包含特殊符号</div>
        </li>
        <li>
          <i class="diandiany"></i>
          <span>账号介绍</span>
        ...
        <!-- <li><span>辅助材料</span><input type="text"></li>
              <li class="li2">
                  <span>辅助资料图片</span>
                  <dl>
                      <dt></dt>
...
export default {
  ...
  methods: {
    ...
    onReads(file) {
      ...
      this.$axios({
        ...
        this.document_imgds = res.data.pojo
      })
    },
    // 扫描件
    onReadsss(file) {
    ...
    handlexieyi() {
      ...
      var openQertyone = {
        ...
        document_img: this.document_img,
        addhanldiqu: this.addhanldiqu,
        document_imgds: this.document_imgds,
        account_introduction: this.account_introduction,
        account_img: this.account_img,
        field: this.field,
        invitation_code: this.invitation_code,
      ...
      window.localStorage.setItem('openQertyone', JSON.stringify(openQertyone))
    },
    // 接收验证码
    yanzhengs: function() {
    ...
    queren(item) {
      ...
      this.addhanldiqu = localStorage.getItem('msgkas')
      this.showone = false
  ...
  created() {
    this.info()
    this.handlestatest()
    var open = JSON.parse(localStorage.getItem('openQertyone'))
    this.name = open.name
  ...
  mounted() {
    ...
    this.$refs.goodsImgss.src = 'http://47.92.85.1/' + open.account_img
  }
}
</script>
