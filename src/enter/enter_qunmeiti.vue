  ...
  <div class="qunmeitia">
    <div class="qunmeiti">
      ...
      <ul>
        ...
        <li id="flag">
          ...
          <input readonly="readonly" onfocus='this.blur()' style="width: 26%;" ref="stylewidth" @click="addBtnFlag && diqu()" placeholder="请选择" v-model="addbtndiqu" />
        </li>
        <li id="free">
          <span></span>
        ...
        <li>
          <i class="diandiant"></i>
          <span>组织机构代码:</span>
          <input id="organ" type="text" v-model="organizational_code" />
        ...
        <li class="li2">
          ...
          <dl>
            ...
            <dd>
              <p>请上传营业执照/组织机构代码证/法人证书扫描件，支持jpg、jpeg、png文件</p>
              <div class="back">
        ...
        <li><span>联系邮箱</span><input type="text"></li>-->
      </ul>
      <div class="ps">
        <div>
          <span>亿源社区信息</span>
      ...
      <ol>
        <li>
          <i class="diandiant"></i>
          <span>账号名称</span>
          <input type="text" v-model="account_title" placeholder="必填项，请输入账号名称" />
        </li>
        <li>
          <i class="diandian"></i>
          <span>账号介绍</span>
      ...
      <ul>
        ...
        <li>
          <span></span>
          <input type="text" v-model="document_number" placeholder="必填项，请输入证件号码" />
        </li>
        <li class="li2">
          <i class="diandian"></i>
          <dl>
    ...
    <div class="checknice">
      <van-checkbox v-model="checked" checked-color="#03a9f4" shape="square">
        同意
        <span @click="handlexieyis">《亿源社区服务协议》</span>
      </van-checkbox>
    </div>
    <div class="div">
...
export default {
  ...
  methods: {
    ...
    onReadss(file) {
      ...
      this.$axios({
        ...
        this.account_imggt = res.data.pojo
      })
    },
    // 运营者照片
    onReads(file) {
      ...
      this.$axios({
        ...
        console.log(res)
        console.log(res.data.pojo)
        this.document_imgss = res.data.pojo
      })
    },
    // 授权书扫描件
    onReadsss(file) {
    ...
    handlexieyis() {
      var openQerqun = {
        ...
        phone: this.phone,
        website_weibo: this.website_weibo,
        account_imggt: this.account_imggt,
        addbtndiqu: this.addbtndiqu,
        document_imgss: this.document_imgss,
        account_introduction: this.account_introduction,
        field: this.field,
        invitation_code: this.invitation_code,
        auxiliary_materials: this.auxiliary_materials,
      ...
      window.localStorage.setItem('openQerqun', JSON.stringify(openQerqun))
      this.$router.push({
        path: '/service'
      })
    },
    ...
    queren(item) {
      ...
      this.addbtndiqu = localStorage.getItem('msgkas')
      this.showone = false
  ...
  created() {
    this.info()
    this.handlestatest()
    var open = JSON.parse(localStorage.getItem('openQerqun'))
    this.name = open.name
    ...
    this.account_imggt = open.account_imggt
    this.document_imgss = open.document_imgss
    this.addbtndiqu = open.addbtndiqu
  ...
  mounted() {
    ...
    this.$refs.goodsImgs.src = 'http://47.92.85.1/' + open.document_imgss
    this.$refs.goodsImgss.src = 'http://47.92.85.1/' + open.account_imggt
  }
}
</script>
